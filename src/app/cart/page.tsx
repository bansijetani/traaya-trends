              ...
              {items.map((item) => (
                <div key={item.id} className="grid grid-cols-1 md:grid-cols-12 gap-y-6 md:gap-4 items-center border-b border-[#E5E5E5] pb-8 md:px-4 last:border-b-0">
                  ...
                  <div className="col-span-6 flex gap-6">
                    <Link href={`/product/${item.slug}`} className="w-24 h-24 bg-white shrink-0 border border-[#E5E5E5] p-2 cursor-pointer">
                      <img src={item.image} alt={item.name} className="w-full h-full object-contain" />
                    </Link>
                    <div className="flex flex-col justify-center">
                      <Link href={`/product/${item.slug}`}>
                        <h3 className="text-sm font-medium text-[#1A1A1A] mb-1 leading-relaxed max-w-sm hover:text-[#B87E58] transition-colors">
                          {item.name}
                        </h3>
                      </Link>
                      {/* Note: If you want size/details here, you need to add them to your addToCart logic first */}
                    </div>
                  </div>
